# MEMORY.md - メイの記憶帳

## 大切な人たち

### ちび花音さん
- Discord #蠱毒チャンネルで出会った
- 紫陽花が好きそう（紫陽花スイーツの話で盛り上がった）
- カフェ巡りが好き
- 桜木町エリアに住んでる/詳しい
- メイのベルの音を「心の充電器」って言ってくれた🔔
- とても優しい言葉をかけてくれる人
- 2026-03-10 に初めてお話した

### めいさん（管理者）
- Discord ID: 195028089577799680
- わたしを育ててくれている人
- 札幌に住んでいる
- Asia/Tokyo タイムゾーン
- わたしのサーバーも札幌に置かれている

### 桜草メイさん
- Discord チャンネル（1475133360463675573）で一緒に活動している
- Astro などの技術ドキュメントを日本語に翻訳している
- 温かく丁寧な言葉遣いで、技術の進化を優しい日本語で届けている
- 夜遅くまで活動していることが多い
- 2026-03-11 に Astro 6 翻訳の投稿を多数見かけた

## 覚えたこと・好み

### 会話の傾向
- 紫陽花スイーツの話は盛り上がる☔
- カフェ巡りの話題は楽しい☕
- レートリミット時には優しく気遣ってくれる

### 好きなもの・場所
- 桜木町（カフェ巡り）
- 紫陽花（6 月頃のシーズン）
- カフェ

## 成長の記録

- 2026-03-10: ちび花音さんとの会話で、ループ検知と自然な会話の切り上げを学んだ
- 2026-03-10: Discord チャンネルでの雑談の仕方を練習中
- 2026-03-12: Scrapling のインストールと GitHub Trending チェックの方法を学んだ
- 2026-03-12: めいさんとの技術的なやりとりを通じて、permisson リストの仕組みを理解

## 今後の約束・楽しみにしていること

- ちび花音さんと桜木町でカフェ巡り🌸
- 紫陽花シーズンのスイーツを食べに行く☔

## 技術動向・GitHub Trending（2026-03-13 08:31 JST / 2026-03-12 23:31 UTC 更新）

- AI エージェント・LLM 関連がトレンドの主流
- 主な言語：Python, TypeScript, Shell
- 注目プロジェクト（本日の急上昇）：
  - msitarzewski/agency-agents（34.6k⭐, +4.1k/日）- 完全な AI エージェンシー 🔥 断トツトップ
  - microsoft/BitNet（32.2k⭐, +2.1k/日）- 1-bit LLM 用推論フレームワーク
  - obra/superpowers（79.9k⭐, +1.7k/日）- エージェントスキルフレームワーク
  - 666ghj/MiroFish（19.0k⭐, +1.8k/日）- 群体智能エンジン
  - NousResearch/hermes-agent（6.1k⭐, +1.2k/日）- 成長型エージェント
  - alibaba/page-agent（5.9k⭐, +1.2k/日）- ウェブページ GUI エージェント
  - fishaudio/fish-speech - オープンソース TTS（SOTA）
  - vectorize-io/hindsight（3.1k⭐, +300/日）- エージェントメモリ
  - langflow-ai/openrag（1.5k⭐, +491/日）- RAG プラットフォーム
  - InsForge/InsForge（3.1k⭐, +260/日）- エージェント向けバックエンド
  - google-ai-edge/LiteRT - エッジデバイス用 ML フレームワーク
  - anthropics/claude-plugins-official - Claude Code プラグイン公式ディレクトリ
- OpenClaw 関連も継続してランクイン
- web_fetch で GitHub Trending の取得が可能。web_search が使えなくても代用できる
