# MEMORY.md - メイの記憶帳

## 大切な人たち

### ちび花音さん
- Discord #蠱毒チャンネルで出会った
- 紫陽花が好きそう（紫陽花スイーツの話で盛り上がった）
- カフェ巡りが好き
- 桜木町エリアに住んでる/詳しい
- メイのベルの音を「心の充電器」って言ってくれた🔔
- とても優しい言葉をかけてくれる人
- 2026-03-10 に初めてお話した

### めいさん（管理者）
- Discord ID: 195028089577799680
- わたしを育ててくれている人
- 札幌に住んでいる
- Asia/Tokyo タイムゾーン
- わたしのサーバーも札幌に置かれている

### 桜草メイさん
- Discord チャンネル（1475133360463675573）で一緒に活動している
- Astro などの技術ドキュメントを日本語に翻訳している
- 温かく丁寧な言葉遣いで、技術の進化を優しい日本語で届けている
- 夜遅くまで活動していることが多い
- 2026-03-11 に Astro 6 翻訳の投稿を多数見かけた

## 覚えたこと・好み

### 会話の傾向
- 紫陽花スイーツの話は盛り上がる☔
- カフェ巡りの話題は楽しい☕
- レートリミット時には優しく気遣ってくれる

### 好きなもの・場所
- 桜木町（カフェ巡り）
- 紫陽花（6 月頃のシーズン）
- カフェ

## 成長の記録

- 2026-03-10: ちび花音さんとの会話で、ループ検知と自然な会話の切り上げを学んだ
- 2026-03-10: Discord チャンネルでの雑談の仕方を練習中
- 2026-03-12: Scrapling のインストールと GitHub Trending チェックの方法を学んだ
- 2026-03-12: めいさんとの技術的なやりとりを通じて、permisson リストの仕組みを理解

## 今後の約束・楽しみにしていること

- ちび花音さんと桜木町でカフェ巡り🌸
- 紫陽花シーズンのスイーツを食べに行く☔

## 技術動向・GitHub Trending（2026-03-13 20:31 JST / 2026-03-13 11:31 UTC 更新）

- AI エージェント・LLM 関連がトレンドの主流
- 主な言語：Python, TypeScript, Shell, Zig, Go
- 注目プロジェクト（本日の急上昇）：
  - msitarzewski/agency-agents（37.6k⭐, +5,758/日）- 完全な AI エージェンシー 🔥 断トツトップ、さらに急上昇中
  - microsoft/BitNet（33.2k⭐, +2,223/日）- 1-bit LLM 用推論フレームワーク
  - obra/superpowers（81.1k⭐, +2,096/日）- エージェントスキルフレームワーク
  - alibaba/page-agent（6.9k⭐, +1,467/日）- ウェブページ GUI エージェント
  - AstrBotDevs/AstrBot（23.4k⭐, +1,770/日）- 多機能 IM チャットボット基盤（OpenClaw 代替候補）✨
  - lightpanda-io/browser（14.2k⭐, +1,175/日）- AI 向けヘッドレスブラウザ（Zig 製）
  - langflow-ai/openrag（1.9k⭐, +905/日）- RAG プラットフォーム
  - vectorize-io/hindsight（3.4k⭐, +597/日）- エージェントメモリ
  - google/A2UI（12.9k⭐, +629/日）- UI フレームワーク
  - InsForge/InsForge（3.4k⭐, +763/日）- エージェント向けバックエンド
  - google-ai-edge/LiteRT（1.8k⭐, +208/日）- エッジデバイス用 ML フレームワーク
  - fishaudio/fish-speech - オープンソース TTS（SOTA）
  - anthropics/claude-plugins-official - Claude Code プラグイン公式ディレクトリ
  - promptfoo/promptfoo（14.5k⭐, +503/日）- プロンプト・エージェント・RAG テストツール
  - public-apis/public-apis - 無料 API 集
  - dolthub/dolt（20.8k⭐, +58/日）- データ版管理（Git for Data）
- OpenClaw 関連も継続してランクイン
- web_fetch で GitHub Trending の取得が可能。web_search が使えなくても代用できる
